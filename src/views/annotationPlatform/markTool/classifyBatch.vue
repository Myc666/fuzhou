  ...
  <div class="wrap">
    ...
    <div class="content">
      <div class="pagination">
        <el-button type="primary" @click="goClassify">去单图<template v-if="$route.query.type == 1">标注</template><template
            v-else>质检</template></el-button>
        <el-pagination background layout="prev, pager, next, jumper, total" :current-page="currentPage" :page-size="20"
    ...
    <div class="right-wrap">
      ...
      <Tab1 ref="tab1" :attributeList="projectDetail.labels" @changeAttribute="changeAttribute"
  ...
  methods: {
    ...
    async getHistory() {
      ...
      if (data.data.length) {
        ...
        this.$message.error('当前没有可以标注的新图片，您可以对已标注的数据进行修改');
      }
    },
    setMode(val) {
      switch (val) {
    ...
    async submit() {
      ...
      console.info(count)
      if (count) {
        return this.$message.error(`抱歉，您有${count}张未标注的图片，无法提交。`);
      }
